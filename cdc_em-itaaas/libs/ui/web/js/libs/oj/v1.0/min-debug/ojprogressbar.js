define(["ojs/ojcore", "jquery", "ojs/ojcomponentcore"], function($oj$$42$$, $$$$40$$) {
  (function() {
    ...
    }, _setOptions:function($options$$311$$) {
      if(!this.options.disabled) {
        var $value$$210$$ = $options$$311$$.value;
        delete $options$$311$$.value;
        this._super($options$$311$$);
        this.options.value = this.$_constrainedValue$($value$$210$$);
        this.$_refreshValue$()
      }
    }, _setOption:function($key$$118$$, $value$$211$$) {
      "max" === $key$$118$$ && ($value$$211$$ = Math.max(this.min, $value$$211$$));
      this._super($key$$118$$, $value$$211$$)
    }, $_percentage$:function() {
      return this.$_indeterminate$ ? 100 : 100 * (this.options.value - this.min) / (this.options.max - this.min)
    }, $_refreshValue$:function() {
      var $value$$212$$ = this.options.value, $percentage$$ = this.$_percentage$();
      this.$valueDiv$.toggle(this.$_indeterminate$ || $value$$212$$ > this.min).toggleClass("oj-corner-right", $value$$212$$ === this.options.max).width($percentage$$.toFixed(0) + "%");
      this.element.toggleClass("oj-progressbar-indeterminate", this.$_indeterminate$);
      this.$_indeterminate$ ? (this.element.attr({"aria-valuetext":"In Progress"}), this.element.removeAttr("aria-valuenow"), this.element.removeAttr("aria-valuemin"), this.element.removeAttr("aria-valuemax"), this.$overlayDiv$ || (this.$overlayDiv$ = $$$$40$$("\x3cdiv class\x3d'oj-progressbar-overlay'\x3e\x3c/div\x3e").appendTo(this.$valueDiv$))) : (this.element.attr({"aria-valuemax":this.options.max, "aria-valuenow":$value$$212$$}), this.$overlayDiv$ && (this.$overlayDiv$.remove(), this.$overlayDiv$ = 
      null))
    }, _destroy:function() {
